...
- 从主题、想法和资料生成创作简报、提纲与正文；联网调研会展示问题、证据维度、时效/冲突提醒、覆盖和缺口，快速/均衡/深入三档只控制执行预算，未覆盖或预算耗尽会明确标为部分调研。用户指定的网页链接会独立保留核验状态，未核验前不会作为写作事实。模型连接可自由配置：预置 OpenAI Codex（自带联网搜索，可单独开关），并支持添加任意 **OpenAI 兼容端点**作为自定义连接（填写 baseUrl、模型名、凭证与代理即可接入 OpenAI API、OpenRouter、Nous Research、NVIDIA Build、GitHub Copilot 等）；独立的联网检索可配置 Tavily 作为检索服务。
