...
文渡是一款面向 Windows 的本地 AI 自媒体工作台。它以用户自己的本地 Markdown 文章库（默认兼容 VitePress，也支持普通 Markdown 内容源）为正式内容源，把资料整理、AI 辅助创作、自然化改写、AIGC 特征检测、封面处理和微信公众号发布串成可恢复的工作流。

项目仍处于早期开发阶段，vibe coding编写，界面、数据结构和安装方式可能持续调整。
...
## 内容源设计：为什么用本地 Markdown

文渡把**用户电脑上的 VitePress Markdown 文章库**作为正式内容源，而不是把内容锁在应用私有的数据库里。这样设计有几个直接好处：

- **工具自由**：文章就是普通的 `.md` 文件。你可以用 Obsidian、VS Code、Typora 等任意编辑器，也可以让其它 AI 智能体、脚本直接在文章库里写稿、改稿——文渡不独占你的内容。
- **多工具协同**：资料整理、写作、AI 辅助可以由不同工具分工完成，文渡负责把终稿统一转换成各平台可发布的形态。
- **可迁移、可备份**：内容以纯文本落在自己的硬盘上，不受单一应用约束，随时可用 Git 或网盘备份、用其它工具打开。
- **创作与发布解耦**：无论文章在哪写成，最后都由文渡处理微信公众号、CSDN、博客园、掘金、51CTO 等渠道的改写、封面、AIGC 检测与发布。

简言之：**文章在你自己的文件里写，文渡只做“最后一公里”的发布。**

## 已实现的主要能力

- 管理多个微信公众号、CSDN、博客园、掘金或 51CTO 账号，并保存账号定位、目标读者、写作风格、禁用话题和常用栏目。
- 扫描本地 Markdown 文章库；配置窗口可选择 VitePress 的 `posts/<文章目录>/index.md` 规则或不带 `posts/` 层级的普通 Markdown 目录。新建草稿直接写入文章库，仍可使用 Obsidian 等外部编辑器。
- 从主题、想法和资料生成创作简报、提纲与正文；联网调研会展示问题、证据维度、时效/冲突提醒、覆盖和缺口，快速/均衡/深入三档只控制执行预算，未覆盖或预算耗尽会明确标为部分调研。搜索结果只用于发现候选，资料卡必须有正文核验或手工摘录；卡片会说明主张、推荐理由、质量/时效、边界及内容指纹，并合并同质来源。用户指定的网页链接会独立保留核验状态，未核验前不会作为写作事实。模型连接可自由配置：预置 OpenAI Codex，并支持添加任意 **OpenAI 兼容端点**作为自定义连接（填写 baseUrl、模型名、凭证与代理即可接入 OpenAI API、OpenRouter、Nous Research、NVIDIA Build、GitHub Copilot 等）；独立的联网检索可配置 Tavily 作为检索服务；“技能与模型”已提供通用联网找图的图片初审模型配置，完整找图闭环按 Issue #19 跟踪。
- 使用所见即所得或 Markdown 原文编辑文章，支持图片、表格、手机预览、选区 AI 编辑、修改对比和文章顾问“阿文”；文章编辑器已支持通过 Tavily 搜索图片候选、按文章保存最近 30 次搜图历史、用 Codex 做可选视觉初审、查看来源并在用户确认后下载插入正文；阿文也能在明确找图意图下调用受控图片工具、结合文章段落推荐插入前后位置并打开候选面板，推荐锚点失效时回退文章末尾；正文对话需要核实网页时由应用侧联网检索后再交给阿文分析，不依赖模型自行直连网页。
- 通过独立技能管理公众号撰写、平台改写、去 AI 味、摘要、封面提示词、封面生成和浏览器检测规则。
- 仓库同时保留 `assets/skills/wechat-video-creator/` 外部可读的视频工作流资源，供其他智能体执行；它尚未注册为文渡内置技能，文渡原生视频生成仍在规划中。
...
- 微信公众号、CSDN、博客园、掘金和 51CTO 的发布任务均可查看统一的只读生命周期轨迹；排查排队、恢复、失败、平台回执和人工校正过程时，底层平台事件仍按各自模型保留。
- 已支持从本地文章库生成、编辑和冻结独立的 CSDN 渠道稿；可选择 AI 改写或直接使用主稿，并拦截公众号链接、文末延伸阅读等软引流内容。CSDN 通过账号隔离的可见浏览器完成登录预检、表单填充和用户最终确认后的单次提交；页面变化或回执不确定时转人工接管，不会静默宣称发布成功。
...
- 已支持掘金账号接入与纯 API 两段式发布：生成并审核掘金渠道稿后，任务先以排队状态落库，再由共享运行器调用 `article_draft/create` 创建草稿；用户确认后调用 `article/publish` 公开，支持 Cookie 凭据恢复、人工校正与幂等防重复。
- 已支持 51CTO 账号接入与渠道稿发布：通过可见浏览器完成登录和 Cookie 获取，读取或手工填写栏目、个人分类和标签，审核冻结后上传本地图片并调用 51CTO 发布接口，支持回执、人工校正和幂等防重复；渠道稿可选择 AI 改写或直接使用主稿。
- 受控代码执行已接入：文章编辑器左侧“代码与工具”会打开宽版执行面板，提供本机、WSL、Docker、Windows Sandbox 目标的前置检查和结构化执行确认；阿文对话可自动连续执行受策略允许的低风险只读，并在右侧“执行活动”展示工作流状态、权限、事件和结果，高风险操作通过授权卡确认。授权目录可按所有文章或当前文章复用，但每次具体命令仍需确认，项目授权默认 24 小时后失效。运行结果、产物哈希与执行记录可追溯，并能只读探测本机 Git、Python、Node、FFmpeg、Pandoc 和 Mermaid CLI。Git 取证支持公开 HTTPS 浅克隆；阿文未收到 staging 目录时会创建受控临时目录并展示实际目标；支持固定 commit、按文件/行号读取源码摘要、插入正文引用、保存待核验证据和单独采纳证据卡。失败工作流不会静默退回旧直答路径，也不会自动把 Git 结果变成文章事实。
- 按日保存运行日志，便于排查模型、微信接口、回调和浏览器自动化问题。
- “技能与模型”中可设置阿文专属工作区；阿文 Git staging 固定写入该目录下的 `git-sources/`，底部对话只跳转到右侧执行活动的唯一授权卡，不再重复显示同一授权。
...
| AI       | 文渡模型适配层与可编辑技能                                                             |
| 发布     | 微信公众号官方 API、回调与必要的浏览器自动化；CSDN 浏览器发布；博客园 MetaWeblog XML-RPC；掘金 content_api；51CTO Cookie/API |
...
npm test                # 自动重建 better-sqlite3 后运行自动化测试
...
```text
assets/skills/   技能资源；manifest.json 中的条目是文渡当前可管理的内置技能，未登记的工作流资源不在文渡界面展示
CONTEXT.md      项目领域词汇与稳定业务术语
docs/            用户与开发者使用文档
scripts/         构建、安装包和验证脚本
...
- 文章正文和图片保存在用户选择的本地 Markdown 文章库；默认是 VitePress 目录结构。
