...
- 受控代码执行已接入：文章编辑器左侧“代码与工具”会打开宽版执行面板，提供本机、WSL、Docker、Windows Sandbox 目标的前置检查和结构化执行确认；授权目录可按所有文章或当前文章复用，但每次具体命令仍需确认。运行结果、产物哈希与执行记录可追溯，并能只读探测本机 Git、Python、Node、FFmpeg、Pandoc 和 Mermaid CLI。Git 取证支持公开 HTTPS 浅克隆、固定 commit、按文件/行号读取源码摘要、插入正文引用和保存实验资料卡；成功运行也会沉淀为受作用域限制的工具记忆。
