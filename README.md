...
- 从主题、想法和资料生成创作简报、提纲与正文；联网调研会展示问题、证据维度、时效/冲突提醒、覆盖和缺口，快速/均衡/深入三档只控制执行预算，未覆盖或预算耗尽会明确标为部分调研。搜索结果只用于发现候选，资料卡必须有正文核验或手工摘录；卡片会说明主张、推荐理由、质量/时效、边界及内容指纹，并合并同质来源。用户指定的网页链接会独立保留核验状态，未核验前不会作为写作事实。模型连接可自由配置：预置 OpenAI Codex，并支持添加任意 **OpenAI 兼容端点**作为自定义连接（填写 baseUrl、模型名、凭证与代理即可接入 OpenAI API、OpenRouter、Nous Research、NVIDIA Build、GitHub Copilot 等）；独立的联网检索可配置 Tavily 作为检索服务；“技能与模型”已提供通用联网找图的图片初审模型配置，完整找图闭环按 Issues #19–#25 分阶段实现。
- 使用所见即所得或 Markdown 原文编辑文章，支持图片、表格、手机预览、选区 AI 编辑、修改对比和文章顾问“阿文”；文章编辑器已支持通过 Tavily 搜索图片候选、按文章保存最近 30 次搜图历史、用 Codex 做可选视觉初审、查看来源并在用户确认后下载插入正文；正文对话需要核实网页时由应用侧联网检索后再交给阿文分析，不依赖模型自行直连网页。
