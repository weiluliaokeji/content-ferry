...
export function App() {
  ...
  type SourceLinkTipSide = "top" | "bottom" | "left" | "right";
  type SourceLinkTipState = { url: string; x: number; y: number; side: SourceLinkTipSide };
  const [sourceLinkTip, setSourceLinkTip] = useState<SourceLinkTipState | null>(null);
  const sourceLinkTipRef = useRef<HTMLDivElement | null>(null);
  const handleSourceLinkEnter = (event: React.MouseEvent<HTMLAnchorElement>, url: string) => {
    const rect = event.currentTarget.getBoundingClientRect();
    const tipW = 320; const tipH = 36; const gap = 8;
    const vw = window.innerWidth; const vh = window.innerHeight;
    // 优先向右（链接左侧常被标题占据，向右通常空间充足），再向左、向下、向上
    let side: SourceLinkTipSide = "right";
    let x = rect.right + gap;
    let y = rect.top + rect.height / 2 - tipH / 2;
    if (x + tipW > vw - 8) { side = "left"; x = rect.left - gap - tipW; y = rect.top + rect.height / 2 - tipH / 2; }
    if (x < 8) { side = "bottom"; x = rect.left + rect.width / 2 - tipW / 2; y = rect.bottom + gap; }
    if (y + tipH > vh - 8) { side = "top"; x = rect.left + rect.width / 2 - tipW / 2; y = rect.top - gap - tipH; }
    setSourceLinkTip({ url, x: Math.max(8, Math.min(x, vw - tipW - 8)), y: Math.max(8, Math.min(y, vh - tipH - 8)), side });
  };
  const handleSourceLinkLeave = () => setSourceLinkTip(null);
  ...
  return <div className={`app-shell${sidebarCollapsed ? " sidebar-collapsed" : ""}`}>
    ...
    {researchProject && (researchGenerating || researchFollowingUp) && researchTaskId && !researchPaused && <div role="status" style={{ position: "fixed", right: 24, bottom: 24, zIndex: 10000, display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", border: "1px solid #cbd5e1", borderRadius: 10, background: "#fff", boxShadow: "0 8px 30px rgba(15, 23, 42, .16)" }}><span>补研正在进行。</span><button type="button" className="secondary-button" onClick={() => void pauseResearch()}>暂停补研</button><button type="button" className="danger-button" onClick={() => void cancelResearch()}>停止</button></div>}
    {researchPaused && researchProject && <div role="status" style={{ position: "fixed", right: 24, bottom: 24, zIndex: 10000, display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", border: "1px solid #cbd5e1", borderRadius: 10, background: "#fff", boxShadow: "0 8px 30px rgba(15, 23, 42, .16)" }}><span>补研已暂停，资料卡已保留。</span><button type="button" className="secondary-button" onClick={() => void resumeResearch()}>继续补研</button></div>}
    ...
    {editingSkill && <Modal onClose={closeSkillEditor} disabled={saving} closeOnBackdrop={false} title={editingSkill.name} eyebrow="技能管理" wide>
    ...
    {(editingConnection || connectionCreating) && <Modal onClose={closeConnectionModal} disabled={saving} closeOnBackdrop={false} title={connectionCreating ? "添加模型连接" : `配置 ${editingConnection?.displayName}`} eyebrow="模型连接">
      <form onSubmit={saveModelConnection} className="profile-form">
        ...
        {connectionCreating && <p className="hint compact-hint">点击窗口外的灰色区域不会关闭此窗口，请使用「取消」或「保存连接」。未保存时关闭会提示确认，避免误触丢失已填内容。</p>}
    ...
    {tavilyModalOpen && <Modal onClose={() => setTavilyModalOpen(false)} disabled={tavilySaving || tavilyTesting} closeOnBackdrop={false} title="配置 Tavily" eyebrow="联网检索服务">
    ...
    {researchProxyModalOpen && <Modal onClose={() => setResearchProxyModalOpen(false)} disabled={researchProxySaving} closeOnBackdrop={false} title="配置检索代理" eyebrow="联网检索服务">
    ...
    {cnblogsCredentialAccount && <Modal onClose={() => setCnblogsCredentialAccount(undefined)} disabled={cnblogsCredentialSaving} closeOnBackdrop={false} title={`配置博客园凭据：${cnblogsCredentialAccount.displayName}`} eyebrow="博客园 MetaWeblog API">
    ...
    {juejinCredentialAccount && <Modal onClose={() => setJuejinCredentialAccount(undefined)} disabled={juejinCredentialSaving} closeOnBackdrop={false} title={`配置掘金凭据：${juejinCredentialAccount.displayName}`} eyebrow="掘金 Cookie + aid + uuid">
    ...
    {fiftyoneCtoCredentialAccount && <Modal onClose={() => setFiftyoneCtoCredentialAccount(undefined)} disabled={fiftyoneCtoCredentialSaving} closeOnBackdrop={false} title={`配置 51CTO 凭据：${fiftyoneCtoCredentialAccount.displayName}`} eyebrow="51CTO 博客 Cookie">
    ...
    {(publishProject || publishSource) && <Modal onClose={() => { setPublishProject(undefined); setPublishSource(undefined); }} disabled={saving || coverGenerating} closeOnBackdrop={false} title={`同步微信草稿：${publishProject?.topic ?? publishSource?.title ?? publishSource?.relativePath}`} eyebrow="第一步只创建草稿" wide>
    ...
    {correctingWechatJob && <Modal onClose={() => setCorrectingWechatJob(undefined)} disabled={wechatCorrectionSaving} closeOnBackdrop={false} title="人工校正微信状态" eyebrow="回执异常兜底">
    ...
    {correctingCsdnJob && <Modal onClose={() => setCorrectingCsdnJob(undefined)} disabled={csdnCorrectionSaving} closeOnBackdrop={false} title="人工校正 CSDN 状态" eyebrow="回执异常兜底">
    ...
    {correctingCnblogsJob && <Modal onClose={() => setCorrectingCnblogsJob(undefined)} disabled={cnblogsCorrectionSaving} closeOnBackdrop={false} title="人工校正博客园状态" eyebrow="回执异常兜底">
    ...
    {correctingJuejinJob && <Modal onClose={() => setCorrectingJuejinJob(undefined)} disabled={juejinCorrectionSaving} closeOnBackdrop={false} title="人工校正掘金状态" eyebrow="回执异常兜底">
    ...
    {correctingFiftyoneCtoJob && <Modal onClose={() => setCorrectingFiftyoneCtoJob(undefined)} disabled={fiftyoneCtoCorrectionSaving} closeOnBackdrop={false} title="人工校正 51CTO 状态" eyebrow="回执异常兜底">
    ...
    {researchProject && <section className="research-workspace-screen" role="region" aria-label={`联网资料：${researchProject.topic}`}>
      ...
      {researchGenerating || researchFollowingUp ? <div className="generation-progress" role="status"><span className="loading-dot" aria-hidden="true" /><span>{researchStatus || "阿文正在检索官方与公开网页，并整理可追溯资料卡…"}</span>{researchTaskId && <button type="button" className="secondary-button" onClick={() => void cancelResearch()}>停止补研</button>}</div> : researchError ? <section className={`research-follow-up research-error-panel research-error-${researchFailureKind ?? "service"}`}><h3>{researchFailureKind === "verification" ? "需要完成网页验证" : researchFailureKind === "network" ? "联网连接失败" : "联网检索未完成"}</h3><p className="error" role="alert">{researchError}</p><p className="hint">{researchFailureKind === "verification" ? "请在“文渡 · 联网检索协助”窗口中完成登录、验证码或其他网页验证；完成后回到这里重试。" : researchFailureKind === "network" ? "这次失败发生在网络或 TLS 连接阶段，不代表资料丢失，也不一定需要网页验证。请稍后重试；如果反复出现，请检查检索代理设置。" : "本轮没有生成可保存的完整结果，已有资料卡和历史运行会保留。请稍后重试，或检查模型与联网配置。"}</p><div className="modal-actions"><button type="button" className="secondary-button" onClick={returnToBriefFromResearch}>返回创作方向</button>{researchTaskId && <button type="button" className="secondary-button" onClick={() => void retryResearchTask(researchTaskId)}>沿用原请求重试</button>}{researchTaskId && !researchPaused && <button type="button" className="secondary-button" onClick={() => void archiveResearchTask(researchTaskId)}>移除记录</button>}{researchFailureKind === "network" && <button type="button" className="secondary-button" onClick={() => { setResearchProject(undefined); setResearch(undefined); setResearchError(""); openResearchProxySettings(); }}>检查检索代理</button>}<button type="button" onClick={() => void openResearch(researchProject, true)}>{researchFailureKind === "verification" ? "完成验证后重试" : "重新连接"}</button></div></section> : !research ? <div className="generation-progress" role="status"><span className="loading-dot" aria-hidden="true" /><span>正在准备资料窗口…</span></div> : <>
        ...
        {!researchReadOnly && (researchProject.outlineReady || researchProject.outlineDraftReady) && <p className="hint compact-hint">已有提纲（{researchProject.outlineReady ? "已确认" : "草稿"}），下方「编辑提纲」只打开它，不会重新生成；若要在提纲里吸收本次新采纳的资料，请使用「按已选资料重新生成提纲」。重新生成后确认前不会覆盖旧提纲，也可在提纲编辑器里「恢复原提纲」。</p>}
        {!researchReadOnly && <div className="research-title-edit-row"><span className="research-title-label">文章标题</span><strong className="research-title-value">{researchProject.topic}</strong><button type="button" className="research-title-edit-button" onClick={() => openTitleEdit(researchProject)}>修改标题</button></div>}
        <div className="modal-actions research-primary-actions">{researchReadOnly ? <button type="button" className="secondary-button" onClick={() => { setResearchProject(undefined); setResearch(undefined); }}>关闭</button> : <><button type="button" className="secondary-button" onClick={returnToBriefFromResearch}>返回创作方向</button><button disabled={researchFollowingUp} onClick={() => { const project = researchProject; setResearchProject(undefined); setResearch(undefined); void openOutline(project); }}>{researchProject.outlineReady ? "编辑提纲" : researchProject.outlineDraftReady ? "继续编辑提纲草稿" : "用已选资料生成提纲"}</button>{(researchProject.outlineReady || researchProject.outlineDraftReady) && <button type="button" className="secondary-button" disabled={researchFollowingUp} onClick={() => { if (window.confirm(`将按最新已采纳资料重新生成提纲。当前提纲（${researchProject.outlineReady ? "已确认" : "草稿"}）在确认新提纲前不会被覆盖；生成后可在提纲编辑器里「恢复原提纲」撤销。是否继续？`)) { const project = researchProject; setResearchProject(undefined); setResearch(undefined); void regenerateOutline(project); } }}>按已选资料重新生成提纲</button>}</>}</div>
      </>}
      </div>
    </section>}
    {titleEditProject && <Modal priority className="title-edit-modal" onClose={closeTitleEdit} disabled={titleEditSuggesting} closeOnBackdrop={false} title="修改文章标题" eyebrow="第三步：补充资料">
      <div className="title-edit-body">
        <p className="title-edit-note">标题会同步更新文章列表与文章目录；若已生成提纲，提纲内的一级标题可在打开提纲后调整。</p>
        <label className="title-edit-field"><span className="title-edit-field-heading"><span>文章标题</span><small>{titleEditText.length}/120</small></span><input autoFocus value={titleEditText} onChange={(event) => setTitleEditText(event.target.value)} maxLength={120} placeholder="可直接填写，或让阿文推荐" /></label>
        <div className="title-edit-suggest-row"><button type="button" className="ai-action-button" onClick={() => void suggestResearchTitle()} disabled={titleEditSuggesting}>{titleEditSuggesting ? "阿文正在推荐…" : "让阿文推荐标题"}</button><small className="hint">推荐结果是候选，点击后才会填入上方标题。</small></div>
  ...
  {sourceLinkTip && createPortal((
    <div
      ref={sourceLinkTipRef}
      className={`source-link-tip source-link-tip-${sourceLinkTip.side}`}
      style={{ position: "fixed", left: sourceLinkTip.x, top: sourceLinkTip.y, zIndex: 9999 }}
      role="tooltip"
