...
function ProjectRow({
  ...
  const nextText = job?.status === "published" ? "文章已确认发布完成" : job?.status === "cancelled" ? "发布任务已取消，可重新设置后再发布" : job?.status === "submitted" ? "已提交微信，等待最终回执" : job?.status === "draft_ready" ? "微信草稿已同步，等待预览和发布" : project.draftReady ? "文章主稿已生成，可继续编辑或发布到其他平台" : project.outlineDraftReady ? "提纲草稿已暂存，可继续编辑或确认就绪" : project.outlineReady ? "文章提纲已确认，下一步生成主稿" : project.researchReady ? "资料已补充，可生成提纲或继续补研" : project.briefReady ? "创作方向已整理，下一步联网补研" : "下一步整理创作方向和资料";
  const action = project.draftReady || project.outlineReady ? () => void openDraft(project) : project.outlineDraftReady ? () => void openOutline(project) : project.researchReady ? () => void openOutline(project) : project.briefReady ? () => void openResearch(project, true) : () => void openBrief(project);
  const label = project.draftReady ? "打开正文" : project.outlineReady ? "起草正文" : project.outlineDraftReady ? "编辑提纲" : project.researchReady ? "生成提纲" : project.briefReady ? "联网补研" : "整理创作方向";
  ...
  const openResearchForFollowUp = async () => {
    await openResearch(project);
    window.setTimeout(() => {
      const input = document.getElementById("research-follow-up-input");
  ...
  return <li key={project.id}>
    <span className="dashboard-row-primary">
      <span className="dashboard-row-title-block">
        <span className="dashboard-row-title-line">
          ...
          <span className="article-title-inline">{project.draftReady ? <button className="article-title-button" onClick={() => void openDraft(project)}>{project.topic}</button> : <strong>{project.topic}</strong>}</span>
          <small className="article-created-at">{formatLocalTimestamp(project.createdAt)}</small>
        </span>
        <span className="dashboard-row-meta"><small className="dashboard-row-status">{nextText}</small><span className="account-context"><span>发布账号：{account ? `${platformName(account.platform)} · ${account.displayName}` : "未选发布账号"}</span></span></span>
      </span>
    ...
    <span className="account-actions">
      ...
      {project.researchReady && <button className="secondary-button" onClick={() => void openResearchForFollowUp()}>继续补研</button>}
      ...
      <button className="danger-button" onClick={() => void deleteProjectDraft(project)} disabled={saving}>{job ? "删除本地文章" : "删除草稿"}</button>
    </span>
  </li>;
}
...
export function DashboardView(props: DashboardViewProps) {
  ...
  return <section className={`card dashboard-card${items.length === 0 ? " dashboard-empty-card" : ""}`}>
    {items.length === 0 ? (
            ...
            <span className="dashboard-row-primary">
              <span className="dashboard-row-title-block">
                <span className="dashboard-row-title-line">
                  ...
                  <span className="article-title-inline"><button className="article-title-button" onClick={() => void openSourceArticle(item.relativePath)}>{item.title ?? "未命名文章"}</button></span>
                  <small className="article-created-at">{formatLocalTimestamp(item.createdAt)}</small>
                </span>
                <span className="dashboard-row-meta"><small className="dashboard-row-status">文章正文可编辑，也可发布到其他平台</small></span>
              </span>
