    ...
    <div className="help-center">
      ...
      <section className="card help-faq">
        ...
        <details>
          ...
          <p>「待处理」只列出尚未确认最终结果的任务；「发布记录」列出已确认终态的任务，包括已发布、已取消，以及你在平台后台核实后人工标记为失败的记录。校正弹框的「最终状态」默认跟随任务当前状态，直接确认即可归档：失败任务确认「发布失败」后会出现在「发布记录」并显示红色「已失败」徽标与核实依据；系统自动判定失败但未经你核实的任务仍留在「待处理」，等待重试或核实。若不再发布，把「最终状态」选为「取消发布」即可。</p>
        </details>
        <details>
        ...
        <details>
          ...
          <p>工作台已有资料的文章会显示“查看资料”和“继续补研”；在“第三步：补充资料”中，可以让阿文继续联网核查，也可只新增指定资料链接。搜索结果只负责发现候选，自动资料卡必须有正文核验摘录；卡片会说明可支持的主张、为什么值得看、质量/时效、证据边界、提取时间和内容指纹，并合并同质来源。AI 推荐和作者决定会分开显示：选择“采纳”后资料才会进入提纲、正文和事实检查；待核验或拒绝的资料不会进入。若合并不准确，可在“资料卡纠错”中拆分来源或合并同一主张，来源和作者决定会保留。资料链接和归因只是写作候选，不会自动插到正文。需要复核会变化的资料时，点击“刷新易变事实”；它只处理产品能力、价格、规则、版本、限额和适用条件，不覆盖已有资料或作者决定。左上角“调研运行历史”可回看每次初始调研、继续补研和刷新当时的计划、结论与资料快照。资料窗口会显示本次问题、证据维度、时效/冲突提醒、已覆盖材料和缺口；应用会按未覆盖维度继续检索，模型自己说“够了”不会提前结束。快速核实、均衡调研、深入研究只改变执行预算，预算耗尽或仍有缺口时会明确显示“部分调研”，可直接围绕缺口继续补研。指定链接会单独显示待人工核验、已人工核验、不采用或无法访问状态；未核验前不会作为写作依据。若已掌握可靠内容，可以填写“手工补录资料卡”；链接与指定资料一致时，这条摘要会成为它的核验结果，保存后资料卡才会默认作为后续提纲和正文的资料依据。补研等待较久时可点“暂停补研”，稍后再点“继续补研”；也可停止本轮任务。已保存的资料卡和历史运行不会丢失；如果应用中途重启，重新打开资料窗口会自动接管排队或已失联的补研任务。</p>
        </details>
        <details>
        ...
        <details>
          ...
          <p>先添加一个掘金账号并配置凭据，再到「工作台」对未归档文章点击掘金平台按钮（已归档文章可在「归档库」点击「重新发布」，在发布设置里选掘金账号后进入掘金渠道稿）。可让阿文生成独立适配稿，也可直接使用主稿而不调用模型；之后可编辑并冻结版本。冻结后创建掘金发布任务，文渡会先创建掘金草稿并展示草稿链接，确认无误后点击「确认公开」才会正式发布。掘金稿不会包含公众号链接、文末延伸阅读等软引流内容。</p>
          <button className="text-button" onClick={() => onNavigate("dashboard")}>去工作台</button>
        </details>
        <details>
          <summary>怎样创建 51CTO 渠道稿？</summary>
          <p>先添加一个 51CTO 账号并配置 Cookie，再到「工作台」对未归档文章点击 51CTO 平台按钮（已归档文章在「归档库」点 ⋯ 展开后点「重新发布」，在发布设置里选 51CTO 账号即可进入渠道稿）。在渠道稿的「发布设置」里先点「加载分类」从 51CTO 拉取你的栏目与分类，然后依次选择「一级栏目」和「授权分类」——两者是联动的，选好一级栏目后，二级授权分类只会列出该栏目下的分类，切换一级栏目时需要重新选择二级分类。再填写「标签」（必填，多个用逗号分隔）。分类拉取失败时也可以直接手动填写栏目 id / 分类 id。点「发布到 51CTO」一步发布，不需要在浏览器里确认。若文章在 51CTO 后台被删除或需要换分类重发，改完后点顶部的「重新发布」即可，会用上次的分类和标签作为默认值。</p>
        </details>
        ...
        <details>
          ...
          <p>在文章编辑器左侧打开“代码与工具”后会显示宽版执行面板。普通 Demo 用结构化参数填写运行时、工作目录和授权目录；分析公开仓库时切换到 Git，填写仓库 HTTPS 地址和新的 staging 目录，选择网络策略后执行“克隆并固定源码版本”，再点“分析仓库文件”读取带 SHA-256 和行号范围的源码摘要。每次运行都会显示实际目标、参数和目录，并留下执行记录；成功结果可保存为带条件的实验资料卡，也可以把源码摘要插入正文引用，不能直接当作通用事实。授权目录可以保存为所有文章共享或仅当前文章使用，但每次具体执行仍需重新确认，不会形成对未知命令的永久信任。</p>
          <p>本机、WSL、Docker 和 Windows Sandbox 是不同执行目标，不可用时文渡会明确提示，不会偷偷回退到本机。安装依赖、删除文件和外部写入仍需另外确认。</p>
        </details>
        <details>
          <summary>文章、会话和阿文记忆保存在哪里？</summary>
          <p>文章保存在 VitePress 文章库；账号设置、发布记录、会话、记忆摘要和日志保存在文渡数据目录。长期记忆只保存提炼后的稳定写作偏好，不把完整会话直接当作写作规则。在正文底部的“与阿文讨论本文”面板中点击“管理本文记忆”，可以查看待确认候选、启用或删除当前文章的记忆，也可以导出 JSON 备份并合并导入；删除派生记忆不会删除文章正文。</p>
        </details>
        <details>
        ...
        <details>
          ...
          <p>选择微信公众号时，留言、原创声明、赞赏和合集等微信专属选项会直接显示，顶栏按钮为「准备发布」；选择 CSDN、博客园、掘金或 51CTO 时，面板会提示「在 XX 渠道稿中发布」，顶栏按钮变为「进入 XX 渠道稿」——这些平台的栏目、分类、标签、发布形式等平台专属设置都在渠道稿里填写，这里保存的作者、摘要和封面会带到渠道稿。</p>
        </details>
        <details>
