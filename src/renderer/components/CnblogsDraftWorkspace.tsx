...
interface CnblogsChannelDraftShape {
...
export type CnblogsDraftPatch = Partial<Pick<CnblogsChannelDraftShape, "title" | "markdown" | "author" | "digest" | "coverSource">>;
...
interface CnblogsPublishOptions {
  categories: string[];
  tags: string[];
}
...
interface CnblogsDraftWorkspaceProps {
  ...
  saving: boolean;
  job?: CnblogsPublishJobShape;
  error?: string;
  onClearError?: () => void;
  onChange: (patch: CnblogsDraftPatch) => void;
  onSave: () => Promise<void> | void;
  onPublish: (options: CnblogsPublishOptions) => void;
...
function cnblogsJobLabel(status: CnblogsPublishJobShape["status"]): string {
  switch (status) {
    ...
    case "needs_credentials": return "需要配置博客园凭据";
  }
}
...
export function CnblogsDraftWorkspace({ draft, accountDisplay, saving, job, error, onClearError, onChange, onSave, onPublish, onConfirmPublish, onCorrectStatus, onGoToCredentials, onDelete, onBack }: CnblogsDraftWorkspaceProps) {
  ...
  const patch = (next: CnblogsDraftPatch) => {
    setDirty(true);
    onChange(next);
  };
  ...
  const handleCorrectStatus = async () => {
    ...
    try {
      ...
      setCorrecting(false);
      setCorrectionReason("已在博客园后台核实");
    } catch (cause) {
      setCorrectionError(cause instanceof Error ? cause.message : "人工校正博客园发布状态失败。");
    } finally {
  ...
  const handleDelete = async () => {
    if (!window.confirm("确定删除这篇博客园渠道稿吗？删除后将从头重新生成，已绑定的发布任务也会一并清除，且不可恢复。")) return;
    setDeleteBusy(true);
    setDeleteError("");
    try {
      await onDelete();
  ...
  const generateSummary = async () => {
    ...
    try {
      const generated = await request<{ summary: string }>("/skills/article-summary/run", {
        method: "POST",
        body: JSON.stringify({ platform: "cnblogs", title: draft.title, markdown: draft.markdown })
      });
  ...
  return <div className="editor-workspace cnblogs-workspace">
    ...
    <div className="editor-columns">
      ...
      <section className={`editor-canvas cnblogs-canvas${editorMode === "markdown" ? " markdown-mode" : ""}`} ref={canvasRef}>
        <p className="hint">当前来源：{draft.generationMode === "source" ? "主稿直接复制（未调用 AI）" : "阿文按博客园调性改写"}。冻结后不能直接修改；主稿变更时应重新生成并审核。</p>
        <label className="cnblogs-title-field">标题<input value={draft.title} maxLength={120} disabled={!isDraft || saving} onChange={(event) => patch({ title: event.target.value })} /></label>
      ...
      <aside className="editor-right-panel">
        ...
        {rightPanel === "settings" && <div className="side-panel-content">
          ...
          {showReconciliationForm && <div className="cnblogs-reconcile-form">
            {!correcting ? <p className="hint">发布结果无法自动确认（回执缺失或异常）。请到博客园后台核对草稿/文章实际状态后人工校正。</p> : <>
              ...
              <label>核实依据（可选）<textarea autoFocus value={correctionReason} onChange={(event) => setCorrectionReason(event.target.value)} maxLength={500} placeholder="例如：在博客园后台“管理”页确认已发布，文章链接为……" /><small>{correctionReason.length}/500，可留空</small></label>
          ...
          <label>发布账号<output className="readonly-account">{accountDisplay}</output><small>该渠道稿已绑定此博客园账号；重新生成可选择其他账号。</small></label>
          <label>作者<input value={draft.author} maxLength={16} disabled={!isDraft || saving} onChange={(event) => patch({ author: event.target.value })} placeholder="用于博客园文章署名" /><small>{draft.author.length}/16 字</small></label>
          <label>摘要
            <textarea value={draft.digest} maxLength={200} disabled={!isDraft || saving} onChange={(event) => patch({ digest: event.target.value })} placeholder="用于内容卡片和分享，最多 200 字" />
          ...
          <div className="settings-cover-section">
            ...
            <button type="button" className="secondary-button" onClick={() => document.getElementById("cnblogs-cover-input")?.click()} disabled={!isDraft || saving || coverBusy}>{coverBusy ? "正在上传…" : "选择本地图片"}</button>
            <input id="cnblogs-cover-input" type="file" accept="image/jpeg,image/png,image/gif,image/webp" hidden onChange={(event) => void chooseCover(event)} />
          ...
          {!isDraft && !job && <p className="hint">渠道稿已锁定为快照。点击顶部的「发布到博客园」会创建博客园草稿（默认不公开），确认后再正式发布。</p>}
          <div className="settings-danger-section">
            <strong>删除渠道稿</strong>
            <small>删除后该渠道稿及已绑定的发布任务都会被清除，需从头重新生成。</small>
            <button type="button" className="danger-button" onClick={() => void handleDelete()} disabled={deleteBusy || saving}>{deleteBusy ? "正在删除…" : "删除这篇博客园渠道稿"}</button>
