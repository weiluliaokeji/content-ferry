...
interface FiftyoneCtoChannelDraftShape {
...
export type FiftyoneCtoDraftPatch = Partial<Pick<FiftyoneCtoChannelDraftShape, "title" | "markdown" | "author" | "digest" | "coverSource">>;
...
interface FiftyoneCtoDraftWorkspaceProps {
  ...
  saving: boolean;
  job?: FiftyoneCtoPublishJobShape;
  error?: string;
  onClearError?: () => void;
...
function fiftyoneCtoJobLabel(status: FiftyoneCtoPublishJobShape["status"]): string {
  switch (status) {
    ...
    case "needs_credentials": return "需要配置 51CTO Cookie";
  }
}

const BLOG_TYPES: Array<{ value: "1" | "2" | "3"; label: string }> = [
  { value: "1", label: "原创" },
  { value: "2", label: "转载" },
  { value: "3", label: "翻译" }
];
...
export function FiftyoneCtoDraftWorkspace({ draft, accountDisplay, saving, job, error, onClearError, onChange, onSave, onPublish, onConfirmPublish, onCorrectStatus, onGoToCredentials, onDelete, onBack }: FiftyoneCtoDraftWorkspaceProps) {
  ...
  const [publishPid, setPublishPid] = useState("");
  const [publishCateId, setPublishCateId] = useState("");
  const [publishTags, setPublishTags] = useState("");
  const [publishBlogType, setPublishBlogType] = useState<"1" | "2" | "3">("1");
  const [correcting, setCorrecting] = useState(false);
  const [correctionStatus, setCorrectionStatus] = useState<"published" | "failed" | "cancelled">("published");
  const [correctionReason, setCorrectionReason] = useState("已在 51CTO 后台核实");
  ...
  const leaveWorkspace = () => {
    if (dirty && !window.confirm("51CTO 渠道稿还有未保存的修改。确定放弃这些修改并返回归档库吗？")) return;
    onBack();
  };

  const patch = (next: FiftyoneCtoDraftPatch) => {
    setDirty(true);
    onChange(next);
  };
  ...
  const handleConfirmPublish = async () => {
    ...
    try {
      ...
      /* 失败由 onConfirmPublish 内部统一 setError 上报 */
    } finally {
  ...
  const handleCorrectStatus = async () => {
    ...
    try {
      ...
      setCorrecting(false);
      setCorrectionReason("已在 51CTO 后台核实");
    } catch (cause) {
      setCorrectionError(cause instanceof Error ? cause.message : "人工校正 51CTO 发布状态失败。");
    } finally {
  ...
  const handleDelete = async () => {
    if (!window.confirm("确定删除这篇 51CTO 渠道稿吗？删除后将从头重新生成，已绑定的发布任务也会一并清除，且不可恢复。")) return;
    setDeleteBusy(true);
    setDeleteError("");
    try {
      await onDelete();
  ...
  const switchToMarkdown = () => setEditorMode("markdown");
    ...
    try {
      const generated = await request<{ replacement: string; conversation?: { userMessage: { content: string }; assistantMessage: { content: string } } }>("/skills/selection-edit/run", {
        ...
        body: JSON.stringify({
          ...
          title: draft.title,
          contextKey: `fiftyone_cto:${draft.id}`,
  ...
  const generateSummary = async () => {
    ...
    try {
      const generated = await request<{ summary: string }>("/skills/article-summary/run", {
        method: "POST",
        body: JSON.stringify({ platform: "51cto", title: draft.title, markdown: draft.markdown })
      });
  ...
  const captureVisualSelection = (selection?: { selectedMarkdown: string }) => {
    setSelectionResult("");
    setAssistantError("");
    if (!selection) {
      setSelectionRange(undefined);
  ...
  useEffect(() => {
    if (rightPanel === "assistant" && !selectionRange) setRightPanel("settings");
  ...
  return <div className="editor-workspace cnblogs-workspace">
    ...
    <header className="editor-topbar">
      ...
      <div className="editor-document-title"><strong>{draft.title || "未命名 51CTO 渠道稿"}</strong><small>51CTO 渠道稿 · {accountDisplay}</small></div>
    ...
    <div className="editor-columns">
      <aside className="editor-left-panel">
        ...
        {leftTool === "structure" && <div className="tool-detail"><strong>文章结构</strong>{headings.length ? headings.map((heading, index) => <button className="structure-link" key={index} style={{ paddingLeft: `${(heading.level - 1) * 10}px` }} onClick={() => undefined}>{heading.text}</button>) : <small>正文中还没有标题。</small>}</div>}
      ...
      <section className={`editor-canvas cnblogs-canvas${editorMode === "markdown" ? " markdown-mode" : ""}`}>
        <p className="hint">当前来源：{draft.generationMode === "source" ? "主稿直接复制（未调用 AI）" : "阿文按 51CTO 调性改写"}。冻结后不能直接修改；主稿变更时应重新生成并审核。</p>
        <label className="cnblogs-title-field">标题<input value={draft.title} maxLength={120} disabled={!isDraft || saving} onChange={(event) => patch({ title: event.target.value })} /><small>{draft.title.length}/120 字 · 51CTO 标题上限 120 字</small></label>
        {editorMode === "visual" ? <Suspense fallback={<p className="hint">正在打开可视化编辑器…</p>}>
          ...
          />
        </Suspense> : <div className="markdown-editor-shell"><div className="markdown-mode-toolbar editor-mode-switch" aria-label="编辑模式"><button type="button" className="editor-mode-icon" title="切换到所见即所得编辑" aria-label="切换到所见即所得编辑" onClick={switchToVisual}>✎</button><button type="button" className="active editor-mode-icon" title="当前：Markdown 原文" aria-label="当前：Markdown 原文">{"</>"}</button></div><textarea ref={markdownSourceRef} className="markdown-source-editor" value={draft.markdown} disabled={!isDraft || saving} onChange={(event) => patch({ markdown: event.target.value })} spellCheck={false} /></div>}
      </section>
      <aside className="editor-right-panel">
        ...
        {rightPanel === "settings" && <div className="side-panel-content">
          <h3>发布设置</h3>
          {assistantError && <p className="error editor-inline-error">{assistantError}</p>}
          {jobStatus === "needs_credentials" && <div className="status-badge warning cnblogs-credential-banner">51CTO 账号尚未配置 Cookie，请先前往账号管理补凭据后重试。</div>}
          ...
          {showReconciliationForm && <div className="cnblogs-reconcile-form">
            {!correcting ? <p className="hint">发布结果无法自动确认（回执缺失或异常）。请到 51CTO 后台核对草稿/文章实际状态后人工校正。</p> : <>
              ...
              <label>核实依据（可选）<textarea autoFocus value={correctionReason} onChange={(event) => setCorrectionReason(event.target.value)} maxLength={500} placeholder="例如：在 51CTO 创作中心页确认已发布，文章链接为……" /><small>{correctionReason.length}/500，可留空</small></label>
          ...
          <label>发布账号<output className="readonly-account">{accountDisplay}</output><small>该渠道稿已绑定此 51CTO 账号；重新生成可选择其他账号。</small></label>
          <label>作者<input value={draft.author} maxLength={16} disabled={!isDraft || saving} onChange={(event) => patch({ author: event.target.value })} placeholder="仅用于本地标识与预览署名" /><small>51CTO 接口不含作者字段，此字段不会随文章发布。</small></label>
          <label>摘要
            <textarea value={draft.digest} maxLength={200} disabled={!isDraft || saving} onChange={(event) => patch({ digest: event.target.value })} placeholder="用于 51CTO 列表摘要，最多 200 字" />
          ...
          <div className="settings-cover-section">
            ...
            <button type="button" className="secondary-button" onClick={() => document.getElementById("fiftyone-cto-cover-input")?.click()} disabled={!isDraft || saving || coverBusy}>{coverBusy ? "正在上传…" : "选择本地图片"}</button>
            <input id="fiftyone-cto-cover-input" type="file" accept="image/jpeg,image/png,image/gif,image/webp" hidden onChange={(event) => void chooseCover(event)} />
            {coverError && <p className="error">{coverError}</p>}
          </div>
          {!isDraft && <p className="status-badge success">已锁定发布快照（如需修改可重新生成渠道稿）</p>}
          {!isDraft && !job && <p className="hint">渠道稿已锁定为快照。点击顶部的「发布到 51CTO」会直接发布到 51CTO。</p>}
          <div className="settings-danger-section">
            <strong>删除渠道稿</strong>
            <small>删除后该渠道稿及已绑定的发布任务都会被清除，需从头重新生成。</small>
            <button type="button" className="danger-button" onClick={() => void handleDelete()} disabled={deleteBusy || saving}>{deleteBusy ? "正在删除…" : "删除这篇 51CTO 渠道稿"}</button>
