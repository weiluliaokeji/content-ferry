...
export function Modal({ title, eyebrow, children, onClose, disabled, wide = false, priority = false, className = "", closeOnBackdrop = true }: { title: string; eyebrow?: string; children: ReactNode; onClose: () => void; disabled: boolean; wide?: boolean; priority?: boolean; className?: string; closeOnBackdrop?: boolean }) {
  return <div className={`modal-backdrop${priority ? " priority-modal" : ""}`} role="presentation" onMouseDown={() => !disabled && closeOnBackdrop && onClose()}><section className={`modal-card${wide ? " wide-modal" : ""}${className ? ` ${className}` : ""}`} role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => event.stopPropagation()}><div className="section-heading"><div>{eyebrow && <p className="eyebrow">{eyebrow}</p>}<h2>{title}</h2></div><button className="text-button" onClick={onClose} disabled={disabled}>关闭</button></div>{children}</section></div>;
}
