...
export function initialiseDatabase(db: Database.Database): void {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS content_practice_plans (
      project_id TEXT PRIMARY KEY REFERENCES content_projects(id) ON DELETE CASCADE,
      markdown TEXT NOT NULL,
      status TEXT NOT NULL CHECK(status IN ('draft', 'confirmed', 'skipped')),
      updated_at TEXT NOT NULL
    );
    ...
    CREATE INDEX IF NOT EXISTS idx_cnblogs_publish_jobs_account_updated
      ON cnblogs_publish_jobs(account_id, updated_at DESC);
    ...
    CREATE INDEX IF NOT EXISTS idx_cnblogs_publish_job_events_job_created
      ON cnblogs_publish_job_events(job_id, created_at DESC);
    ...
    CREATE INDEX IF NOT EXISTS idx_juejin_publish_jobs_account_updated
      ON juejin_publish_jobs(account_id, updated_at DESC);
  ...
  if (!accountProfileColumns.some((column) => column.name === "fiftyone_cto_pid_options")) {
    db.exec("ALTER TABLE account_profiles ADD COLUMN fiftyone_cto_pid_options TEXT NOT NULL DEFAULT ''");
  }
