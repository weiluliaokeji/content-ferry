...
- 分类与标签：渠道稿创建先用固定分类 ID / 已知 tag 映射快速填充；AI 推荐在后台补充，发布设置仍可人工修改，未命中按 ID 透传。
- 软引流拦截（复用 CSDN 的拦截规则）。
- 发布状态管理与人工校正。
...
- 标签：创建渠道稿时先用已知 tag 名 → ID 映射（AI编程 / OpenAI / AIGC）快速填充；AI 会在后台从官方标签池推荐更贴合的结果，失败时保留确定性推断。未命中的输入按 tag ID 原文透传。
