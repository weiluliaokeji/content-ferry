...
统一生命周期事件由 `PublishLifecycleService` 追加写入 `publish_lifecycle_events`，通过 `GET /api/publish-lifecycle/jobs/:jobId/events` 只读查询；发布中心的终态记录展示倒序轨迹。该查询只返回规范状态、来源、原因和时间，不暴露凭据、正文或平台鉴权细节，也不提供外部副作用操作。
