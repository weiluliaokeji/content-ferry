...
跨平台发布由共享 lifecycle module 负责本地任务的幂等、冻结快照引用、规范状态迁移、重试、人工校正和追加式事件记录；平台 adapter 只负责平台差异：内容转换、凭据/登录、素材处理、提交和回执读取。第二阶段已建立 `publish_lifecycle_jobs` 与 `publish_lifecycle_events` 统一表，四个平台的 adapter 在创建、状态迁移和人工校正时同步写入统一记录；原有平台任务表继续保留，作为平台细节和升级兼容层。
...
当前实现状态（2026-09-07）：已接入共享 lifecycle module 的 CSDN adapter seam。独立渠道稿、AI 改写或主稿直接派生、编辑、软引流拦截、审核冻结、快照哈希、幂等键、规范状态映射，以及受控可见浏览器的登录预检、表单填充、用户最终确认、单次提交、远端回执读取和人工校正均已实现。CSDN 的 `needs_login`、`needs_user`、`ready_for_final_confirmation` 等内部状态会分别映射为凭据阻塞或等待用户操作；提交回执不确定时进入待人工核对。剩余风险：CSDN 编辑器 DOM 选择器与发布按钮文案以假设为主，需在真实 CSDN 账号下做端到端验证；验证通过前，填充/回读失败会安全降级为人工接管，不会静默宣称发布成功。
...
博客园适配器已接入共享渠道发布生命周期 module：冻结渠道稿后以标题、正文、摘要、作者、封面、分类/标签和素材引用计算快照哈希，兼容升级前幂等键；`draft_creating`、`draft_created`、`confirming`、`needs_credentials`、`failed` 等平台状态分别映射为规范状态。草稿创建、公开失败和凭据阻塞仍可在原任务恢复；已发布、已取消或待人工核对任务不被普通重试复用。删除本地渠道稿时只软删除草稿并保留发布任务及事件历史。
...
掘金适配器已接入共享渠道发布生命周期 module：冻结渠道稿后以标题、正文、摘要、作者、封面、分类/标签和素材引用计算快照哈希，并兼容升级前幂等键；`draft_creating`、`draft_created`、`confirming`、`needs_credentials`、`failed` 等平台状态映射为规范状态。草稿创建、公开失败和凭据阻塞仍可在原任务恢复；已发布、已取消或待人工核对任务不被普通重试复用。删除本地渠道稿只软删除草稿，发布任务和事件历史继续保留。
...
51CTO 适配器已接入共享渠道发布生命周期 module。渠道稿与主稿、微信公众号稿、CSDN 稿保持独立；流程为：主稿派生 → 用户编辑标题、摘要、标签、栏目和分类 → 冻结版本与设置快照 → 校验账号 Cookie → 通过可见浏览器读取或手工填写栏目/授权分类 → 将本地图片上传到 51CTO 图床并转换正文 HTML → 单次调用 51CTO 发布接口 → 回写远端 URL/内容 ID 或进入人工校正。单阶段提交仍保留 `draft_creating` 作为执行中的平台状态，结果映射为 `published`、`failed`、`needs_credentials` 或 `needs_manual_reconciliation`。
