...
- `research_runs` 以 `research_tasks.id` 为幂等锚点保存每次成功的初始、继续或时效刷新调研快照：计划、结论、资料卡（含正文快照和作者决定）及指定资料均序列化后只写入一次。当前研究表仍服务编辑与写作；刷新只追加不重复的新资料，失败、取消、暂停或预算耗尽不会写运行快照，也不会破坏当前资料。应用以“刷新易变事实”的固定指令优先重核产品能力、价格、规则、版本、限额和适用条件。
- 自动资料卡不把搜索摘要当作证据：`ConfiguredModelProvider` 在搜索后逐 URL 提取正文，只有非空正文才会进入综合提示；综合模型只能从这些正文摘录对应的 URL 中选择卡片。应用保存不超过 1600 字的正文快照、提取时间和 SHA-256，模型给出的主张、推荐理由、质量/时效与边界连同同质来源 URL 一并写入 `content_research_sources.evidence_json`。手工卡保存输入摘录的同类快照，并明确标记为手工资料。卡片按证据类型和相同主张合并，没有产品层面的固定数量上限；作者决定独立存为 `adoption_status`（推荐、已采纳、已拒绝、待人工核验），刷新资料不会覆盖决定，只有已采纳卡的完整证据元数据会装配进提纲、正文和事实检查上下文。
