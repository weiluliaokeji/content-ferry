...
- 默认关闭。在“技能与模型 → 模型连接 → AI 调用审计”中开启后，每次文本或图片模型调用都会把**完整的请求与响应**写入数据目录下的 `<dataDir>/logs/ai-audit/ai-audit-YYYY-MM-DD.log`（JSON 行，每行一次调用，含 task、skillId、provider、model、token 用量、耗时、ok/error、完整 prompt 与响应）。异步图片任务会在同一条记录中保留提交、轮询和下载请求的响应摘要；鉴权头、密钥、带签名的 URL 查询参数和二进制图片不写入。
