...
阿文的每条可应用正文建议随对应会话消息持久化。建议必须保存 operation：replace 替换唯一原文，insert_before 或 insert_after 则把建议内容作为独立段落插入原文所在段落前/后并保留原文；旧数据缺少该字段时兼容为 replace。接受建议先只修改当前编辑草稿，并保留“已应用但尚未保存”的本地状态；正文文件保存成功后，客户端再调用本地 API 将建议标记为已应用并保存。保存失败或用户放弃未保存修改时，不得把建议标记为已应用；重新打开文章时仍显示为待处理。正文修改后不再能唯一定位原文时，客户端须移除该建议并调用本地 API 更新为无法定位。历史版本若出现“已接受”但建议尚未按其 operation 应用，应自动修复为待处理，避免把未落盘的建议隐藏。建议浮窗的位置只保存相对其唯一正文锚点的用户拖动偏移，不保存绝对屏幕坐标，以保证编辑模式切换和窗口尺寸变化后的定位稳定。
