...
掘金当前仍使用平台侧生命周期实现，尚未接入本次共享 `PublishTaskModule`；现有冻结渠道稿、快照哈希、幂等键、平台状态映射和人工核对语义保持不变。后续迁移时应复用同一 adapter seam，并保持路由、DTO 和平台明细表兼容。
...
51CTO 当前仍使用平台侧生命周期实现，尚未接入本次共享 `PublishTaskModule`。渠道稿与主稿、微信公众号稿、CSDN 稿保持独立；现有 Cookie 校验、可见浏览器接管、素材上传、单次提交、回执和人工校正语义保持不变。后续迁移不得改变单阶段提交的“结果不确定即人工核对”规则。
