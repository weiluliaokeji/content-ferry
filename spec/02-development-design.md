...
图片初审设置保存在 `app-settings.json`，包括 `imageReviewMode`（`disabled`、`current`、`specific`）和可选的 `imageReviewProvider`。设置页面只允许选择启用且不是明确不支持图片输入的连接；视觉能力未知时必须在界面展示待验证与规则筛选回退说明。当前 `/api/image-candidates/search` 只启用 Tavily 的 `include_images`，不把 Bing/DuckDuckGo 的文字结果误当图片；`/api/image-candidates/review` 会用既有 `RemoteImageImportService` 做安全下载，再把临时文件交给已配置的视觉模型，完成后删除临时文件，不写入文章素材。编辑器候选面板展示初审结论，但确认后才调用既有 `RemoteImageImportService` 写入文章素材；`/api/image-candidates/preview` 只为候选大图预览提供安全代理，不会写入正文或素材目录。`/api/image-candidates/history` 以 `contextKey` 为文章范围读取历史，`POST` 在搜索与初审完成后保存最终候选，数据库每篇文章最多保留最近 30 次搜索。当前联网找图确认后统一插入文章 Markdown 末尾，前后保留空行，并在界面明确提示该位置。自定义视觉连接和阿文 `find_images` 工具按 GitHub Issues #21、#25 继续增强。
