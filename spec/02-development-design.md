...
微信公众号“原创、赞赏、合集”不依赖未验证的公开 API。文章设置在创建草稿时复制到 `wechat_publish_jobs` 的设置快照（`declare_original`、`enable_reward`、`collection_name`）；发布记录启动浏览器辅助时只读取该快照，避免后来编辑文章设置改变一份已创建的草稿。可见浏览器先唯一定位草稿标题，再对可识别的开关执行幂等设置；无法确定控件状态、存在同名合集或页面结构变化时不猜测、不点击，保留页面、显示接管提示并由用户完成最后预览与发布。
