...
**已知边界（用户反馈）：** 跨模型解耦已完成（见 `spec/02-development-design.md` §3.2）：联网补研已与单一模型解耦——检索由应用自有 `WebSearchClient`（Tavily / DuckDuckGo 零密钥兜底）承担，`ConfiguredModelProvider.webResearch` 在任意已配置文本模型上完成多轮检索规划（方案 A 工具调用 / 方案 B 规划器）与综合；Codex 仅在综合阶段作为普通模型使用，其 SDK 内置检索已关闭。原"补研必须用 Codex"的护栏已移除。下一步待办仍包括：必须登录 / 验证码 / 动态页面的可见浏览器补研、官方来源域名白名单与来源去重、可信度提示、手工补录资料卡、引用到正文的精确映射。
