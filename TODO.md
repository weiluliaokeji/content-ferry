...
**状态：** 待确认（2026-09-05 提出，未实现）

`useChannelOperations.channelRowsFor` 已为每个平台准备好「生成 XX 稿 / 进入 XX 稿」操作，工作台通过 `<ChannelStrip actions />` 使用它们；归档库当前按 `spec/05-product-interaction-redesign.md` §4.1「归档库只负责状态查阅，工作台额外显示创建/继续按钮」不传 `actions`，重发只能走 ⋯ →「重新发布」→ 发布设置按平台交接渠道稿。

**动机：** 用户从归档库重发 51CTO 时，需要先打开发布设置、切账号、再点「进入 51CTO 渠道稿」，比工作台多两步；渠道条上直接给按钮更直观。

**待决策：** 开放后归档库与工作台的职责边界（查阅 vs 操作）会弱化，需先与 §4.1 的设计意图对齐——是放宽 §4.1，还是保持归档库只读、仅优化交接路径的引导文案。

**完成条件：** 决策明确后，同步更新 `spec/05` §4.1、`docs/USER-GUIDE.md` 归档库章节与 `HelpCenter` 的对应条目；若开放，需一并确认渠道条在归档库窄行内的布局不挤占 ⋯ 按钮。
...
**已知边界（用户反馈）：** 跨模型解耦已完成（见 `spec/02-development-design.md` §3.2）：联网补研已与单一模型解耦——检索由应用自有 `WebSearchClient`（Tavily / Bing RSS / DuckDuckGo 回退链）承担，`ConfiguredModelProvider.webResearch` 在任意已配置文本模型上完成多轮检索规划（方案 A 工具调用 / 方案 B 规划器）与综合；Codex 仅在综合阶段作为普通模型使用，其 SDK 内置检索已关闭。原"补研必须用 Codex"的护栏已移除。下一步待办仍包括：必须登录 / 验证码 / 动态页面的可见浏览器补研、官方来源域名白名单与来源去重、可信度提示、手工补录资料卡、引用到正文的精确映射。
