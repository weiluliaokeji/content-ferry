...
**已知边界（用户反馈）：** 当前"联网资料补研"被强制要求使用 OpenAI Codex，因为实时网页检索（`webSearchMode: "live"`）由 Codex SDK 内置，其它 provider 路径（`chat/completions`、Copilot）没有联网能力。`configured-model-provider.ts` 在调用前以护栏 fail-fast。若要让补研"跨模型通用"，需要把"搜索 + 抓取网页"这一步改为由应用自己承担（搜索 API + 页面抓取），再把结果喂给任意模型，而不是委托给 Codex SDK。这是一笔比本次大的重构，**尚未排期**；在此之前仍保持"补研必须用 Codex"并在 UI/审计中给出明确原因。
